...
function sparkline(product: ReleasedProduct): string {
  const points = [...product.history].reverse().slice(-16);
  if (points.length < 2) return '<div class="spark-empty">販売データ待ち</div>';
  const max = Math.max(...points.map((point) => point.demand), 1);
  const path = points.map((point, index) => `${index ? 'L' : 'M'}${index / (points.length - 1) * 100},${38 - point.sold / max * 34}`).join(' ');
  const demand = points.map((point, index) => `${index ? 'L' : 'M'}${index / (points.length - 1) * 100},${38 - point.demand / max * 34}`).join(' ');
...
function allocationEditor(state: GameState, product: ReleasedProduct): string {
  ...
  return `<div class="allocation-editor"><h4>生産割り当て</h4>${contracts.map((contract) => {
    const factory = getFactoryDefinition(contract);
    const value = product.factoryAllocations.find((allocation) => allocation.contractId === contract.id)?.allocation ?? 0;
    return `<label><span><b>${esc(factory?.name ?? '工場')}</b><small>${contract.setupRemaining ? `立上げ残り${contract.setupRemaining}経営週` : `上限 ${contract.committedCapacity.toLocaleString()}個/5秒`}</small></span><input type="number" min="0" max="${contract.committedCapacity}" step="50" value="${value}" data-product-allocation="${product.id}" data-contract="${contract.id}"></label>`;
  }).join('') || '<p>契約中の工場がありません。</p>'}</div>`;
}

function productCard(state: GameState, productBase: ReleasedProduct): string {
  const product = productBase as RuntimeProduct;
  ...
  const inventoryCash = product.inventory * product.unitCost;
  const plannedLife = Math.max(20, (product.endOfLifeAt.year * 52 + product.endOfLifeAt.week) - (product.launchedAt.year * 52 + product.launchedAt.week));
  const remainingLife = Math.max(0, plannedLife - (product.businessAgeWeeks ?? 0));
  ...
  return `<article class="product-card card ${product.status}"><header><div>${badge(product.status === 'selling' ? '販売中' : product.status === 'eol' ? '終売' : '発売準備', product.status === 'selling' ? 'good' : '')}<h3>${esc(product.name)}</h3><p>${product.category.toUpperCase()} · 第${product.generation}世代 · ${esc(product.audience[0]?.name ?? '')}</p></div><div class="rating"><b>${product.rating.toFixed(1)}</b><small>/10</small></div></header>
    <div class="sales-hero"><div><small>直近5秒販売</small><b>${product.weeklySales.toLocaleString()}</b><span>需要 ${product.weeklyDemand.toLocaleString()}</span></div><div><small>生産</small><b>${product.weeklyProduced.toLocaleString()}</b><span>供給率 ${pct(supplyRate)}</span></div><div class="${lost > 0 ? 'bad' : ''}"><small>機会損失</small><b>${lost.toLocaleString()}</b><span>累計 ${product.lifetimeLostSales.toLocaleString()}</span></div><div class="${moneyTone(product.weeklyProfit)}"><small>直近利益</small><b>${yen.format(product.weeklyProfit)}</b><span>1個粗利 ${yen.format(margin)}</span></div></div>
    ${sparkline(product)}
    ...
    <dl class="cost-list"><div><dt>価格</dt><dd>${yen.format(product.price)}</dd></div><div><dt>原価</dt><dd>${yen.format(product.unitCost)}</dd></div><div><dt>在庫</dt><dd>${product.inventory.toLocaleString()}個</dd></div><div><dt>在庫資金</dt><dd>${yen.format(inventoryCash)}</dd></div><div><dt>製品サイクル</dt><dd>残り約${remainingLife}経営週</dd></div></dl>
...
export function renderProducts(state: GameState): string {
  const selling = state.products.filter((product) => product.status !== 'eol');
  ...
  return `<div class="panel-section"><header class="section-head"><div><small>PRODUCT PERFORMANCE · 5 SEC BUSINESS WEEK</small><h2>商品評価と売れ行き</h2><p>5秒ごとに需要・供給・販売・利益が更新されます。技術年とは独立しています。</p></div></header><div class="summary-strip"><div><small>総需要</small><b>${demand.toLocaleString()}</b></div><div><small>総生産</small><b>${produced.toLocaleString()}</b></div><div><small>総販売</small><b>${sold.toLocaleString()}</b></div><div class="${moneyTone(profit)}"><small>製品利益</small><b>${yen.format(profit)}</b></div></div>
...
function contractCard(state: GameState, contractId: string): string {
  ...
  return `<article class="factory-card card"><header><div>${badge(contract.setupRemaining ? '立上げ中' : '稼働中', contract.setupRemaining ? 'warning' : 'good')}<h3>${esc(factory.name)}</h3><p>${esc(factory.country)} · ${factory.nodeClass}nm級 · ${esc(factory.description)}</p></div><b>${contract.committedCapacity.toLocaleString()} / 5秒</b></header><div class="metric-quad"><div><small>品質</small><b>${factory.quality}</b>${bar(factory.quality)}</div><div><small>信頼性</small><b>${factory.reliability}</b>${bar(factory.reliability)}</div><div><small>割当済み</small><b>${allocated}</b>${bar(allocated, contract.committedCapacity, allocated > contract.committedCapacity ? 'bad' : '')}</div><div><small>最低期間</small><b>${contract.remainingWeeks}経営週</b></div></div><footer><label>契約枠<input type="number" min="100" max="${factory.weeklyCapacity}" step="50" value="${contract.committedCapacity}" data-contract-capacity="${contract.id}"></label><button class="danger-text" data-action="cancel-contract" data-id="${contract.id}">解約</button></footer></article>`;
}
...
function companyCard(company: CompetitorCompany): string {
  ...
  return `<article class="competitor-card card ${company.status}"><header><div><h3>${esc(company.name)}</h3><p>${status} · 戦略 ${esc(company.strategy)}</p></div><b>${pct(company.marketShare * 100)}</b></header><div class="mini-metrics"><span>技術 <b>${num.format(company.technology)}</b></span><span>ブランド <b>${num.format(company.brand)}</b></span><span>勢い <b>${num.format(company.momentum)}</b></span><span>資金 <b>${yen.format(company.cash)}</b></span></div><div class="rival-models">${company.models.slice(-3).reverse().map((model) => `<div><b>${esc(model.name)}</b><small>${model.category.toUpperCase()} · 性能 ${num.format(model.performance)} · ${yen.format(model.price)}</small></div>`).join('') || '<small>現行モデルなし</small>'}</div></article>`;
}
...
export function renderCompetitors(state: GameState): string {
  ...
  return `<div class="panel-section"><header class="section-head"><div><small>COMPETITIVE LANDSCAPE</small><h2>競合企業とモデル</h2><p>強い既存企業と新興勢力が同じ市場で競争します。分類ラベルではなく、技術・ブランド・勢い・製品で判断します。</p></div></header><div class="market-headline card"><b>${esc(state.market.headline)}</b><span>次の市場変化まで約${state.market.weeksRemaining}経営週</span></div><h3>性能上位モデル</h3><div class="rival-table">${current.map((model, index) => `<div><b>${index + 1}</b><span>${esc(model.name)}</span><small>${model.category.toUpperCase()}</small><strong>${num.format(model.performance)}</strong><em>${yen.format(model.price)}</em></div>`).join('')}</div><h3>企業</h3><div class="competitor-list">${state.competitors.map(companyCard).join('')}</div></div>`;
}
