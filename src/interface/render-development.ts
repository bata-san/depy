...
function renderDraft(state: GameState, series: ProductSeries, model: InterfaceModel): string {
  ...
  if (!draft || draft.seriesId !== series.id) {
    return `<section class="empty-state"><span class="eyebrow">NEXT GENERATION</span><h3>次世代設計を開始</h3><p>社員構成と技術投資が開発速度・品質・不具合率を直接変えます。</p><button class="action-primary" data-action="new-design">設計を始める</button></section>`;
  }
  ...
  return `<section class="design-workspace">
    <div class="design-inputs">
      <div class="view-subhead"><div><span class="eyebrow">DESIGN INPUT</span><h3>${esc(series.name)} Gen ${series.generation + 1}</h3></div><span>${series.category.toUpperCase()}</span></div>
      ...
      <button class="action-primary action-wide" data-action="start-generation">開発開始 <span>${yen.format(metrics.developmentCost * .28 + 2_200_000)}</span></button>
    </div>
