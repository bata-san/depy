...
function teamView(state: GameState): string {
  ...
  return `<section class="company-pane"><div class="team-power"><div><span>開発力</span><b data-team-engineering>×${engineering.toFixed(2)}</b></div><div><span>販促力</span><b data-team-sales>×${salesStaffEffect(state).toFixed(2)}</b></div><div><span>生産管理</span><b data-team-operations>×${operationsStaffEffect(state).toFixed(2)}</b></div></div><div class="section-label"><span>STAFF</span><b>${state.staff.length}</b></div><div class="stack-list">${state.staff.map(staffLine).join('')}</div><div class="section-label"><span>RECRUITING</span><b>${state.candidates.length}</b></div><div class="market-lines">${state.candidates.map((candidate) => `<article class="market-line candidate-line" data-candidate-card="${candidate.id}"><div><span>${esc(ROLE_LABELS[candidate.role])} · ${esc(SPECIALTY_LABELS[candidate.specialty])}</span><h3>${esc(candidate.name)}</h3><div class="candidate-stats"><b>能力 ${Math.round(candidate.skill)}</b><b>発想 ${Math.round(candidate.creativity)}</b><b>規律 ${Math.round(candidate.discipline)}</b><b>成長 ${Math.round(candidate.growth)}</b></div><small>${candidate.traits.map((trait) => esc(TRAIT_LABELS[trait])).join(' / ')}</small></div><div class="market-metrics"><span>契約金 <b>${yen.format(candidate.signingBonus)}</b></span><span>給与 <b>${yen.format(candidate.salary)}/月</b></span><span data-candidate-expiry>残り ${Math.max(0, candidate.expiresAtWeek - state.absoluteWeek)}週</span></div><button class="action-primary" data-action="hire" data-id="${candidate.id}">採用</button></article>`).join('') || '<section class="empty-state">次の採用市場更新を待っています。</section>'}</div></section>`;
}

function facilitiesView(state: GameState): string {
  return `<section class="company-pane"><div class="stack-list">${facilityIds.map((id) => { const definition = FACILITY_DEFINITIONS[id]; const facility = state.facilities.find((entry) => entry.id === id); const level = facility?.level ?? 0; const cost = Math.round(definition.baseUpgrade * Math.pow(1.72, level)); return `<article class="line-card facility-line"><header><div><span class="status-text">Lv.${level}</span><h3>${esc(definition.name)}</h3><small>${esc(definition.description)}</small></div><strong>${yen.format(definition.weeklyMaintenance * level)}/週</strong></header><div class="level-track">${Array.from({ length: 5 }, (_, index) => `<i class="${index < level ? 'is-on' : ''}"></i>`).join('')}</div><button class="action-primary" data-action="facility-upgrade" data-id="${id}">Lv.${level + 1}へ強化 · ${yen.format(cost)}</button></article>`; }).join('')}</div></section>`;
}
...
export function renderCompany(state: GameState): string {
  ...
  return `<section class="view"><header class="view-head"><div><span class="eyebrow">COMPANY</span><h2>会社</h2></div><p>社員が開発・研究・販売・生産の主役です。</p></header><nav class="subnav">${tabs.map(([id, label]) => `<button class="${state.companyTab === id ? 'is-active' : ''}" data-action="company-tab" data-tab="${id}">${label}</button>`).join('')}</nav>${content}</section>`;
}
